...
class SampleServer(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if session is None:
            ...
            self.send_header("Content-type", "text/html")
            self.send_header(
                "Location", f"/{html_page}?sessionKey={session['session_key']}")
            self.end_headers()
            ...
            # Request to get /api/v2/users
            me = get_users(session['access_token'])
            if me is None:
                self.send_response(404)
                self.wfile.write(bytes("404: Not found", "utf-8"))
            else:
                ...
                self.end_headers()
                self.wfile.write(me)
        elif self.path.startswith('/user_convo'):
            # Request to get /api/v2/users
            me = get_convo(session['access_token'])
            if me is None:
                self.send_response(404)
                self.wfile.write(bytes("404: Not found", "utf-8"))
            else:
...
def get_me(token):
    ...
    response = requests.get(
        f"https://api.{ENVIRONMENT}/api/v2/users/me", headers=request_headers)
...
def get_users(token):
    ...
    # Get user
    response = requests.get(
        f"https://api.{ENVIRONMENT}/api/v2/users?pageSize=100", headers=request_headers)
...
def get_convo(token):
    # Prepare for GET /api/v2/conversations request
    request_headers = {
        'Authorization': 'Bearer ' + token
    }
    ...
    # Get user
    response = requests.get(
        f"https://api.{ENVIRONMENT}/api/v2/conversations?communicationType=Call", headers=request_headers)
