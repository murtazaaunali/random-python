...
query.action = "EXPORT"
query.action_date = "2023-09-14T13:00:00.000Z"
query.include_screen_recordings = "false"
...
if job_state == 'READY':
    try:
        ...
        print(
            f"Successfully execute recording bulk job { execute_job_response}")
    except ApiException as e:
        print(f"Exception when calling RecordingApi->put_recording_job: { e }")
        sys.exit()
else:
    print(
        f"Expected Job State is: READY, however actual Job State is: { job_state }")
...
#         print(
#             f"Successfully cancelled recording bulk job { cancel_job_response}")
#     except ApiException as e:
#         print(
#             f"Exception when calling RecordingApi->delete_recording_job: { e }")
#         sys.exit()
...
try:
    ...
    print(
        f"Successfully get recording bulk jobs { get_recording_jobs_response}")
except ApiException as e:
